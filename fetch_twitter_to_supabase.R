...
cookie_json   <- Sys.getenv("TW_COOKIES_JSON")  # GH secret (string)
...
## 3 – scrape ----------------------------------------------------
handles <- strsplit(Sys.getenv("TW_HANDLES",
...
tweet_to_list <- function(tw, user) {
  ...
  list(
    ...
    reply_count=rep, retweet_count=rt, like_count=like,
    quote_count=quo, bookmarked_count=bok, view_count=view,
    date=py_str(tw$date),
...
scrape_one <- function(user, limit = 150L) {
  coro <- api$user_by_login(user); info <- asyncio$run(coro)
  coro2 <- twscrape$gather(api$user_tweets_and_replies(info$id, limit = limit))
  tweets <- asyncio$run(coro2)
  purrr::map_dfr(0:(py_len(tweets)-1),
...
con <- DBI::dbConnect(
  RPostgres::Postgres(), host = supa_host,
  port = as.integer(Sys.getenv("SUPABASE_PORT", "5432")),
  dbname = Sys.getenv("SUPABASE_DB", "postgres"),
  user = supa_user, password = supa_pwd, sslmode = "require"
)
